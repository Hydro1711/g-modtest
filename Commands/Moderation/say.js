  ...
  async execute(interaction) {
    ...
    if (
      ...
      return interaction.reply({
        content:
          "<a:Pretend_tick:1278355749843767348> You need the **Manage Messages** permission to use this command.",
        ephemeral: true
      });
    }
    ...
    try {
      const response = await interaction.awaitModalSubmit({
        time: 300000,
        filter: i =>
          i.customId === "say_modal" && i.user.id === interaction.user.id
      });

      const title = response.fields.getTextInputValue("title").slice(0, 256);
      const description = response.fields
      ...
      const embedToggle = response.fields
        .getTextInputValue("embed_toggle")
        .toLowerCase();

      // @everyone/@here mention check
      ...
      if (embedToggle === "on") {
        ...
        try {
          ...
          if (
            imageUrl &&
            /^https?:\/\/.+\.(png|jpg|jpeg|gif|webp|bmp)$/i.test(imageUrl)
          ) {
          ...
          return response.reply({
            content:
              "<a:Pretend_cross:1278355750913183754> Invalid color provided. Please use a valid hex color or color name (e.g. Blue).",
            ephemeral: true
          });
        }
      ...
      await response.reply({
        content:
          "<a:Pretend_tick:1278355749843767348> Your message has been sent!",
        ephemeral: true
      });
    } catch (error) {
